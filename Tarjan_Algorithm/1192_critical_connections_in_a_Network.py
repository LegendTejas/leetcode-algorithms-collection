...
Find all edges that are bridges in the given undirected network graph.
This implementation uses the low-link (Tarjan) technique for bridges.

Time Complexity:  O(V + E)
    - Each node and edge is visited once during DFS traversal.

Space Complexity: O(V + E)
    - Adjacency list, recursion stack, lows array, and output list.

Where:
    V = number of nodes (n)
    E = number of edges (len(connections))
"""
